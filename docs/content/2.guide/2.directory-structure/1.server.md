...
The inbuilt handler can directly return JSON data, a `Promise` or use `event.res.end()` to send response.
...
export default ((event) => `Hello, ${event.context.params.name}!`)
```
...
export default (() => 'Test get handler')
```

```ts [server/api/test.post.ts]
export default (() => 'Test post handler')
```
...
export default (() => `Default foo handler`)
```

```ts [server/api/[...].ts]
export default (() => `Default api handler`)
```
